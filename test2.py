...
def init_supabase_database(supabase_url: str, supabase_key: str, db_password: str) -> SQLDatabase:
    ...
    try:
        ...
        logger.info(f"Attempting to connect to Supabase database: {host}:{port}/{database}")
...
def get_enhanced_sql_chain(db):
    ...
    llm = AzureChatOpenAI(
        azure_endpoint=os.get_env_var("AZURE_OPENAI_ENDPOINT"),
        api_key=os.get_env_var("AZURE_OPENAI_API_KEY"),
        api_version=os.get_env_var("AZURE_OPENAI_API_VERSION"),
        azure_deployment=os.get_env_var("AZURE_OPENAI_DEPLOYMENT_NAME"),
        temperature=0,
        max_tokens=800,
    )
...
def get_enhanced_response(user_query: str, db: SQLDatabase, chat_history: list):
    ...
    try:
        ...
        llm = AzureChatOpenAI(
            ...
            api_key=os.get_env_var("AZURE_OPENAI_API_KEY"),
            api_version=os.get_env_var("AZURE_OPENAI_API_VERSION"),
            azure_deployment=os.get_env_var("AZURE_OPENAI_DEPLOYMENT_NAME"),  # Changed from deployment_name
...
with st.sidebar:
    ...
    default_url = os.get_env_var("SUPABASE_URL", "")
    default_anon_key = os.get_env_var("SUPABASE_ANON_KEY", "")
    default_db_password = os.get_env_var("SUPABASE_DB_PASSWORD", "")
