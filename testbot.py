...
def get_env_var(key, default=None):
    """Get environment variable from Streamlit secrets or OS with better error handling"""
    try:
        # For Streamlit Cloud - try nested structure first
        if hasattr(st, 'secrets'):
            if key == "AZURE_OPENAI_ENDPOINT" and "azure_openai" in st.secrets:
                return st.secrets["azure_openai"]["endpoint"]
            elif key == "AZURE_OPENAI_API_KEY" and "azure_openai" in st.secrets:
                return st.secrets["azure_openai"]["api_key"]
            elif key == "AZURE_OPENAI_API_VERSION" and "azure_openai" in st.secrets:
                return st.secrets["azure_openai"]["api_version"]
            elif key == "AZURE_OPENAI_DEPLOYMENT_NAME" and "azure_openai" in st.secrets:
                return st.secrets["azure_openai"]["deployment_name"]
            elif key == "SUPABASE_URL" and "supabase" in st.secrets:
                return st.secrets["supabase"]["url"]
            elif key == "SUPABASE_ANON_KEY" and "supabase" in st.secrets:
                return st.secrets["supabase"]["anon_key"]
            elif key == "SUPABASE_DB_PASSWORD" and "supabase" in st.secrets:
                return st.secrets["supabase"]["db_password"]
            # Fallback to flat structure
            elif key in st.secrets:
                return st.secrets[key]
    except Exception as e:
        logger.warning(f"Error accessing secrets for {key}: {str(e)}")
