...
def detect_visualization_request(user_query: str):
    ...
    # Check for efficiency-related queries (usually single bar chart)
    efficiency_keywords = ['efficiency', 'performance', 'utilization', 'productivity']
    is_efficiency_query = any(keyword in user_query_lower for keyword in efficiency_keywords)
    ...
    if is_efficiency_query and ('each machine' in user_query_lower or 'machine' in user_query_lower):
        # Single bar chart for machine efficiency comparison
...
def create_enhanced_visualization(df, chart_type, user_query):
    ...
    try:
        ...
        # NEW: Single bar chart for machine efficiency
        if chart_type == "efficiency_bar":
            # Look for efficiency-specific column patterns
            machine_col = None
            efficiency_col = None
            ...
            for col in df.columns:
                if col.lower() in ['efficiency_percent', 'efficiency', 'performance', 'utilization']:
                    efficiency_col = col
                    break
            ...
            logger.info(f"Detected columns for efficiency - Machine: {machine_col}, Efficiency: {efficiency_col}")
            
            if machine_col and efficiency_col:
                # Create single bar chart for machine efficiency
                fig = px.bar(
                    df, 
                    x=machine_col, 
                    y=efficiency_col,
                    title=f"Machine Efficiency Comparison",
                    labels={
                        machine_col: "Machine",
                        efficiency_col: "Efficiency (%)" if 'percent' in efficiency_col.lower() else "Efficiency"
                    },
                    color=efficiency_col,
                    color_continuous_scale='RdYlGn',  # Red-Yellow-Green scale for efficiency
                    text=efficiency_col
                )
                ...
                fig.update_layout(
                    ...
                    title_font_size=16,
                    yaxis=dict(range=[0, max(df[efficiency_col]) * 1.1])  # Add some space above bars
                )
                
                # Add horizontal line for average efficiency
                avg_efficiency = df[efficiency_col].mean()
                fig.add_hline(
                    y=avg_efficiency, 
                    line_dash="dash", 
                    line_color="red",
        ...
        elif chart_type == "multi_machine_bar" or chart_type == "grouped_bar":
